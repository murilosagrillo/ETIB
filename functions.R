...
qetib <- Vectorize(function(u,alpha,beta,lambda,phi){
  val <- ifelse(lambda!=0, qbetapr((1+lambda-sqrt(((1+lambda)^2)-4*u^(1/phi)*lambda))/(2*lambda),alpha,beta), ((1-qbeta(u^(1/phi),alpha,beta))^(-1))-1)
  return(val)
})
...
retib <- function(n,alpha,beta,lambda,phi){
 ...
 if(lambda!=0)
  ...
  {val <-((1-qbeta(u^(1/phi),alpha,beta))^(-1))-1}
    return(val)
}
...
"etibfit"<-function(y){
  ...
  cdfETIB_ks_test<-function(y,par){
    ((1 + lambda_emv)* ((pbetapr (y, alpha_emv, beta_emv))) - lambda_emv*((pbetapr (y, alpha_emv, beta_emv)))^2)^phi_emv
  }
